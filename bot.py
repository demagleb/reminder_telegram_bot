...
from keyboards import DefaulfKeyboard, EditListKeyboard,\
    TimeKeyboard, ListKeyboard
import langs
import states
import config
...
async def process_new_reminder_time(message: types.Message, state: FSMContext):
    ...
    await message.answer(lang.ENTER_TIME,
                         reply_markup=TimeKeyboard(lang))


@dp.message_handler(state=states.NewReminder.time)
async def process_new_reminder_reminder(message: types.Message, state: FSMContext):
    lang = langs.get_lang(message.from_user.language_code)
    ...
    async with state.proxy() as data:
        sql.insert_reminder(dict(user_id=message.from_user.id,
                            text=data['text'], time=t, period=0))
        await message.answer(lang.REMINDER_WRITED.format(datetime.datetime.fromtimestamp(t)),
                             reply_markup=DefaulfKeyboard(lang))
    await state.finish()
...
async def process_new_periodic_reminder_reminder(message: types.Message, state: FSMContext):
    lang = langs.get_lang(message.from_user.language_code)
    reminder = message.text
    await state.update_data(text=reminder)
...
async def process_new_periodic_reminder_first_time(message: types.Message, state: FSMContext):
    lang = langs.get_lang(message.from_user.language_code)
...
async def process_new_periodic_reminder_period(message: types.Message, state: FSMContext):
    lang = langs.get_lang(message.from_user.language_code)
    ...
    async with state.proxy() as data:
        sql.insert_reminder(dict(user_id=message.from_user.id,
                                 text=data['text'], time=data['first_time'], period=period))
...
def build_list(reminders, start, lang):
    ...
    for rem in enumerate(reminders[start:start+8], start+1):
        if not rem[1]['period']:
            x_text = rem[1]['text'] if len(
                rem[1]['text']) <= 40 else rem[1]['text'][:40] + '...'
            text += lang.LIST_ITEM.format(
                id=rem[0],
                time=datetime.datetime.fromtimestamp(rem[1]['time']),
                text=x_text)
        else:
            text += lang.PERIOD_LIST_ITEM.format(
                id=rem[0], time=datetime.datetime.fromtimestamp(
                    rem[1]['time']),
                text=rem[1]['text'], period=lang.timeformat(rem[1]['period']))
    return text
...
async def process_show_list_command(message: types.Message):
    ...
    keyboard = ListKeyboard(
        1, len(reminders), 0, len(reminders) > 8, lang)
    await message.answer(text, reply_markup=keyboard)


@dp.callback_query_handler(lambda q: q.data and q.data.startswith('list-change-page'))
async def process_list_callbacks(callback_query: types.CallbackQuery):
    ...
    keyboard = ListKeyboard(
        ind+1, min(ind + 9, len(reminders)), ind > 0, len(reminders) > ind + 8, lang)
    await callback_query.message.edit_text(text)
    if text != lang.YOUR_LIST_EMPTY:
        await callback_query.message.edit_reply_markup(keyboard)


@dp.callback_query_handler(lambda q: q.data and q.data.startswith('list-item-menu'))
async def process_list_item_menu(callback_query: types.CallbackQuery):
    ...
    if index > len(reminders):
        ...
        return
    await callback_query.message.edit_text(lang.LIST_ITEM.format(
        time=datetime.datetime.fromtimestamp(reminders[index-1]['time']),
        text=reminders[index-1]['text']))
    await callback_query.message.edit_reply_markup(EditListKeyboard(index, lang))


@dp.callback_query_handler(lambda q: q.data and q.data.startswith('list-item-delete'))
